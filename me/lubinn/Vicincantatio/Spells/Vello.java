...
class Vello extends Spell {
	public Spell CastSpell(PlayerChatEvent event, Material mat, int size, Map<String, Boolean> augments) {
		...
		for (Iterator iterator = entities.iterator(); iterator.hasNext();) {
			...
			if ((location.distance(entity.getLocation()) > 5.0D) || ((!Vicincantatio.config.getBoolean("vello_affects_players", true)) && ((entity instanceof Player)))
					|| ((!Vicincantatio.config.getBoolean("vello_affects_self", true)) && (event.getPlayer().getEntityId() == entity.getEntityId())))
				continue;
